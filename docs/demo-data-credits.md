...
| 加工 | 左上の題字「日本地図（47都道府県）」の囲みを削除。**地図の外側を透明化**（ページの地色に馴染ませるため）。256色に減色（1178KB → 177KB） |
...
>
> 作り直しに使った道具一式と手順は、元画像と同じ場所（本人の Downloads の
> `japan-map-component/mapgen/`）に置いてあります。**リポジトリには入れていません**
> — 一度きりの変換で、依存も無い Go の小さなスクリプトのためです。
