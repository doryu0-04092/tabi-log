...
### 7. 応答ヘッダーは CloudFront で全経路に付ける

画面も API も画像も同じ配信元から出る。**経路ごとに抜けがあると、
抜けている経路が狙われる。** `response_headers_policy` を3つの
behavior すべてに付ける。

| ヘッダー | 何を防ぐか |
|---|---|
| HSTS | 1回目以降の平文アクセス。転送はしているが、**その1回目は平文で飛ぶ** |
| X-Content-Type-Options | 画像として上げたものがスクリプトとして解釈される経路 |
| X-Frame-Options: DENY | クリックジャッキング |
| Referrer-Policy | 遷移先に投稿 ID を渡すこと |
| CSP | **外部からのスクリプト読み込み** |

**CSP の要点は `script-src 'self'` である。** 外部の CDN から
スクリプトを読めなくする。読めると、その CDN が改ざんされた時点で
同一オリジンから API を呼ばれる。

`unsafe-inline` は許している。SvelteKit が起動用のスクリプトを HTML に
直接書き込むためである。**外すには SvelteKit 側の csp 設定でハッシュを
埋める必要があり、それは別の変更として行う。** 外部スクリプトを塞ぐ
という主目的はこのままでも果たせている。

**`connect-src` に画像バケットを入れること。** アップロードは
ブラウザから S3 へ直接 PUT する。入れ忘れると画像の送信だけが
動かなくなり、**エラーはブラウザのコンソールにしか出ない。**

### 8. 画像処理の失敗は DLQ で気づく

S3 → Lambda は非同期で、**呼び出し側は結果を知らない。** 既定では
2回再試行したあと、失敗はどこにも残らずに捨てられる。画面は22秒で
諦め、`media` は pending のまま残る。

**利用者には「処理中のまま終わらない画像」に見え、こちらには何も
届かない。** 失敗した呼び出しを SQS へ送り、滞留にアラームを置く。

**通知先は既定で空にしてある。** 送り先を決めずに通知の仕組みだけ
作ると、鳴っていることに誰も気づかないまま「監視している」つもりに
なる。`alert_email` を渡したときだけ SNS のトピックと購読を作る。

拾い直す仕組みは入れていない。**まず気づけるようにするのが先である。**

### 9. 溜まり続けるものはアプリケーションの中で片付ける

期限切れのトークンの行、投稿されなかった画像、消し損ねた S3 の
オブジェクト。**どれも放っておいても例外は出ない。** 行が増え、
オブジェクトが増え、費用と検索の重さだけが静かに悪化する。

専用のバッチや Lambda は立てず、API サーバーの中で1時間ごとに回す。
**タスクが複数あると同じ掃除が重なって走るが、削除は冪等なので
害が無い。** 排他のために分散ロックを足すほうが、この規模では
複雑さに見合わない。

```
期限切れトークン   expires_at < 今 - 24時間  → 消す
孤児になった画像   post_id が空 かつ アバターでもない かつ 8日以上前
                   → S3 を消してから行を消す（順序を逆にしない）
削除待ちの控え     pending_object_deletions  → 消せたときだけ控えを消す
```

**失効済みでも期限内のトークンは残す。** 盗用の検知は「失効済み
トークンの再提示」で判定しており、行を消すと再提示が「知らない
トークン」に見えて検知できなくなる。

### 10. Terraform の state はローカル管理のまま
...
| レート制限 | アプリのメモリ内 | **タスクを増やすと厳密に効かなくなる**（いまは 1 タスクなので効いている）。ElastiCache か WAF で行う |
| Terraform state | ローカル | S3 バックエンド + ロック |
| CD | 手動デプロイ | GitHub Actions から ECR push / ECS デプロイ / S3 sync |

**レート制限の項目は、割り切りの中でも影響が具体的である。**
アプリのメモリで数える実装は、タスクが複数あると各タスクが独立に数えるため、
実効的な上限がタスク数ぶん増える。

**いまは `desired_count` が 1 なので厳密に効いている**（2026-09-01 に実測、
`desired: 1 / running: 1`）。つまり現時点で破れてはいない。

**危ないのはタスク数を増やしたときである。** 上のスケールの項目に沿って
2 以上にすると、そのぶん上限が緩む。とくに効くのはログイン試行の
10回/5分で、2タスクなら 20回/5分になる。**増やす判断とセットで、
外部ストアか WAF への移行を決めること。**
