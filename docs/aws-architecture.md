...
>
> また、**画像の配信は本書の記述（CloudFront + 署名付き Cookie）と実装が食い違っている。**
> アプリケーションは現在 S3 の署名付き URL を返しており（`internal/storage/s3.go`）、
> CloudFront の `/images/*` を作っても誰も通らない。
> **使われない設定は置いていない**（`infra/cloudfront.tf` の末尾に移行時の手順を記載）。
