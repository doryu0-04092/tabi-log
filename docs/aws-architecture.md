...
> **現在の状態**: **Terraform のコードは `infra/` にある**（2026-08-29）。
> `terraform fmt` と `terraform validate` は CI で回しているが、
> **`plan` も `apply` も実行していない。** 本書と `infra/README.md` は
> 実装前の設計であり、「動いた記録」ではない。適用したら実測結果を追記する。
>
