...
> **`INDEX(user_id, id DESC)` だけは後から足した（000003）。** 初回は
> `INDEX(user_id, created_at DESC)` で足りると考えていたが、
> `WHERE user_id = ? AND id < ? ORDER BY id DESC` は絞り込みにしか使えず、
> `EXPLAIN` に `Using filesort` が出た（2026-08-29 に実測）。
> **カーソルを `id` だけにする方針なら、絞り込みの列と `id` を並べた索引が要る。**
> 新着フィード（絞り込み無し）は主キーの逆順走査でそのまま解決できていたため、
> この違いに気づいたのは利用者ごとの一覧を作った時点だった。
>
> **同じ形の修正を3回した**（000003 posts / 000004 notifications /
> 000005 posts の訪問日）。カーソルの列の並びと索引の列の並びは対で決まる、
> というのがこの設計の要点である。
