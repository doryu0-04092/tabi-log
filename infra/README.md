...
> **状態: 2026-08-31 に `apply` して稼働中。** 2026-09-01 に CD で更新している。
> 手順と結果は `docs/aws-architecture.md`、
> 検証の記録は `docs/deploy-verification.md` を参照。
>
> ```
> Plan: 86 to add, 0 to change, 0 to destroy.
> ```
>
> **86 は 2026-08-31 の apply の実績である。** 以前ここに書いていた 74 は
> 8-30 時点の plan の数で、その後 DLQ・アラーム・掃除まわりを足したぶん増えている。
> **数が合わないまま残すと、どちらが正しいのか分からなくなる。**
>
> | 確かめられたこと | 結果 |
> |---|---|
...
**2026-08-31 に初回実行が成功している**（`docs/progress.md` の「済んだこと」）。
そのときに OIDC の `sub` の形式とイメージの取り違えで2回落としており、
経緯は `docs/progress.md` の落とし穴に残してある。

書いた時点で確認できていたのはここまでだった。
...
> **`apply` と画像の CloudFront 配信は 2026-08-31 に確認済み。**
> 下の2項目は書いた当時の状態で、いまは解消している。経緯を残すために消していない。
